...
Look at the effect of body mass and sex on rank for 3 day bin prior the social perturbation experiment (rank assessment point 1). 

Linear regression of rank ~ body mass * sex 
...
exp_capture_dates <- c("2021-05-19", "2021-06-05", "2021-06-23", "2021-07-05")

# size for only capture 1
size<- read.csv("./data/morphometrics_focals_2021.csv") %>%
  dplyr::select(date, id, weight) %>%
...
ec_size_sex <- ec_rankXbin %>% 
  filter(bin==1) %>% 
  left_join(size_sex, by="id") %>%
  dplyr::select(date, id, sex, weight, rank, dom_ec, power) 
...
# difference between body mass and sex for time point 1
t.test(weight~sex, ec_size_sex)

summ_mass <- ec_size_sex %>% 
  group_by(sex) %>%
  summarize(mean= mean(weight, na.rm=TRUE),
            sd = sd(weight, na.rm=TRUE)) 

# difference between rank and sex
...
ggplot(data = ec_size_sex , aes(sample = power)) +
  stat_qq() +
  stat_qq_line()


m <- lm(power ~ weight, data =ec_size_sex)
summary(m)
...
ec_size_sex[c(20), ]# no outlier

# which distribution?
range(ec_size_sex$power, na.rm=TRUE)
...
ggplot(ec_size_sex, aes(x = weight, y=power)) +
  geom_point() +
  theme_classic() +
  labs(x="body weight (g)", y="power score") +
...
## outliers -> excluded instead of taking another weight
...
size <- size_all %>%
  filter(!weight %in% c(72.82, 86.80, 121.4)) %>% # 4 captures during social perturbation experiment) 
  drop_na() %>%
