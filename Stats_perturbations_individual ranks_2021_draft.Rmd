...
###1. correlation body mass and rank
...
plot_corr <- ggplot(power_size, aes(x = weight, y = power, shape = sex)) +
  geom_point() +
  theme_classic() +
  facet_grid(~trial) +
  #geom_text(aes(label=rank),  nudge_x=0.44, nudge_y=0.001) +
...
We did not find a clear pattern that at a specific time point body mass and power were correlated. Plus, as we found that body mass does not influence rank, could it be that body mass is an outcome of rank. We could test this by correlating power change and weight change for each trial. Does initial weight predict rank after initial group formation period? 

###2. does body mass predict rank?
regression of (weight at 2021-04-05) on power (ordinal rank in supplemental)
...
plot_priorweight <- ggplot(start_size_pwr, aes(x=weight, y=power, color= sex, shape=sex)) +
  geom_point() +
  theme_classic() +
...
plot_change <- ggplot(change, aes(x=weight_change, y=power_change, shape=sex, color=rank)) + 
  geom_point() + 
  facet_wrap(~trial) +
  ...
  geom_rect(aes(xmin = 0, xmax = Inf, ymin = 0, ymax = Inf),
                   fill = "grey", alpha = 0.01, color=alpha("grey",.01)) +
  geom_rect(aes(xmin = -Inf, xmax = 0, ymin = -Inf, ymax = 0),
                   fill = "#dc8580", alpha = 0.01, color=alpha("#dc8580", 0.01)) +
  scale_color_viridis(option = "D", direction = -1) #palette = "Spectral")
...
plot_masschangefocal_removal <- focal_long %>%
  ...
  #facet_wrap(~period) +
  labs(x= "perturbed bird ID", y= "weight change (%)", title="following removal") +
  theme_classic() +
...
ggarrange(plot_corr,
          ...
          ncol=1, nrow=2, 
          labels = c("(a)", "(b)"))
ggsave("figures/Fig.2.pdf" ,width = 7, height = 5, units = "in")
```
...
ggsave("figures/Fig.3.pdf",width = 7, height = 3, units = "in")
