...
## power score
ec_sex <- ec_rankXbin %>%
  left_join(birdID_select) %>% # if not summarizing: msex <- glmmadmb(power ~ sex + (1|bin), ec_sex); summary(msex)
  group_by(sex, id) %>%
  summarise(mean=mean(power))
ec_sex$bin <- as.factor(ec_sex$bin)
wilcox.test(mean ~ sex, ec_sex)


msex <- glmmadmb(power ~ sex + (1|bin), ec_sex)
...
## beta regression, if we also want to control directly for the confound factor, sex (yet using this method we do find sex effect)
beta_int <- betareg(power ~ weight * sex, ranksize_stable)
beta_full <- betareg(power ~ weight + sex, ranksize_stable)
beta_mass <- betareg(power ~ weight, ranksize_stable)
beta_sex <- betareg(power ~ sex, ranksize_stable)
beta_null <-betareg(power ~ 1, ranksize_stable)
AIC(beta_int, beta_full, beta_sex, beta_mass, beta_null)

AIC(beta_int, beta_null)
lrtest(beta_full, beta_sex)
summary(beta_int)
...
plot_pwr_weight_bin1 <- ggplot(ranksize_stable, aes(x = weight, y=power , color = sex)) +
  geom_point() +
  theme_classic() +
  labs(x="body weight (g)", y="power score") 
...
```{r rank ~ prior body mass: change close to one another}
...
weight_pwr1 <- left_join(weight_t1, power_t2, by="id")

## beta distribution
ggqqplot(weight_pwr1, "power")

cor.test(weight_pwr1$weight,weight_pwr1$rank )
...
ggplot(weight_pwr1, aes(x=rank, y=weight)) +
  geom_point() +
  theme_classic() +
  labs(y="body weight at time 1 (g)",x="power score at time 2")
...
power_t4 <- ec_rankXbin %>%
  filter(bin == 10) # last rank assessment point (we don't have bin 13 due to tropical storm)
...
## combine dfs
pwr_wght <- bind_rows(weight_pwr1, weight_pwr2,weight_pwr3) 
pwr_wght <- filter(pwr_wght, power!="NA")

ggplot(pwr_wght, aes(x=weight, y=power, color=id)) +
  geom_point() +
  theme_classic() +
  labs(x="prior body weight (g)",y="power score") +
  geom_smooth(method="lm", se=F)

pwr_wght$type <- as.factor(pwr_wght$type.x)
pwr_wght$id <- as.factor(pwr_wght$id)


mfull <- glmmadmb(power ~ weight + (1|type), pwr_wght)
mint <- glmmadmb(power ~ weight * type, pwr_wght)
mnull <- glmmadmb(power ~ 1 + (1|type), pwr_wght)
summary(mfull)
summary(mint)
summary(mnull)
lrtest(mnull, mfull)
```
If looking at the relationship between prior body mass on rank per trial separately, then only trial 1 shows a positive relationship, where heavier birds have higher ranks in the 3 days afterwards and I found no relationship for trial 2 or 3. 
If we combine the 3 trials and control for the multiple measures by adding a random factor, trial, I found no significant relationship (LRT: 0.002, p = 0.96; GLMM:0.001 +- 0.02, random factor variance < 0.001)



```{r rank ~ body mass: change far in between}
...
label ordinal rank on the points and include mean power and weight change lines
